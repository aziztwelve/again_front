...
> Актуально с 2026-07-31: подарок всегда выбирается по каждой применённой
> акции. Переключателя «подарок или промокод» нет. Поле промокода показывается,
> если `allow_promo_codes = true` хотя бы у одной применённой акции; промокод
> суммируется с подарками. После пересчёта суммы подарок, выпавший из порога,
> остаётся в интерфейсе неактивным с пояснением и не уходит в заказ.

На фронте (корзина + чекаут) реализована логика:
1. Проверка применимых акций при изменении корзины
2. Отображение подарка по каждой применённой акции
3. Показ поля промокода, если его разрешает хотя бы одна применённая акция
...
| Файл | Описание |
|------|---------|
| `components/Cart/Promotion.vue` | Блок подарков по применённым акциям |
...
- [ ] `allow_promo_codes = true` хотя бы у одной акции → поле промокода показано вместе с подарками
