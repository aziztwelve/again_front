...
Подарок и промокод больше не являются альтернативами: переключатель
«Подарок / Промокод» на витрине отсутствует, покупатель всегда выбирает подарок
по каждой применённой акции.

- Если **хотя бы у одной** применённой акции `allow_promo_codes = true`, поле
  промокода показывается и промокод можно применить вместе со всеми подарками.
- Если у всех применённых акций флаг выключен, поле промокода не показывается.
- После применения или удаления промокода пересчитывается `cart.total`, затем
  заново вызывается `checkApplicable()` и набор подарков сверяется с новой суммой.
- Подарок, который был доступен до промокода, но выпал из условия по новой
  сумме, не исчезает: он отображается неактивным с пояснением. Такой подарок не
  передаётся в `promotions[]` и не добавляется к заказу.

Например, при стекируемых акциях от 1 000 ₽ и 4 000 ₽ промокод может оставить
подарок от 1 000 ₽ доступным, а подарок от 4 000 ₽ показать заблокированным.
